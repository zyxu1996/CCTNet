...
class ASPPPlusHead(nn.Module):
    def __init__(self, num_classes, in_channels, norm_layer=norm_layer, up_kwargs=up_kwargs, in_index=[0, 3]):
        ...
        self.channels = in_channels // 2 ** in_index[1]
        self.aspp = ASPP_Module(in_channels, [12, 24, 36], norm_layer=norm_layer, up_kwargs=up_kwargs)
        self.c1_block = _ConvBNReLU(self.channels, self.channels, 3, padding=1, norm_layer=norm_layer)
        self.block = nn.Sequential(
            _ConvBNReLU(self.channels + in_channels // 8, self.channels + in_channels // 8, 3, padding=1, norm_layer=norm_layer),
            nn.Dropout(0.5),
            _ConvBNReLU(self.channels + in_channels // 8, self.channels + in_channels // 8, 3, padding=1, norm_layer=norm_layer),
            nn.Dropout(0.1),
            nn.Conv2d(self.channels + in_channels // 8, num_classes, 1))
